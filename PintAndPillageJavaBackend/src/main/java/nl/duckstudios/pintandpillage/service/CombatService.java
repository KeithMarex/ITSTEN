...
public class CombatService {
    ...
    public void checkHasEnoughUnitsToAttack(List<VillageUnit> attackingUnits, Village attackingVillage) {
        for (VillageUnit attackingUnit : attackingUnits) {
            ...
            attackingVillage.getUnitInVillage(attackingUnit.getUnit().getUnitName()).setAmount(unitInVillage.getAmount() - attackingUnit.getAmount());
        }
    }

    public void checkHasEnoughShipsToSendUnits(List<VillageUnit> villageUnits) {
        int totalShipCapacity = villageUnits.stream()
