...
public abstract class ResearchBuilding extends Building {
    ...
    @Getter
    @Setter
    private LocalDateTime currentResearchFinishTime = null;

    @Getter
    @Setter
    private LocalTime researchTimeLeft = null;
    ...
    public void startResearch(Research research) {
        ...
        this.currentResearchFinishTime = LocalDateTime.now().plusSeconds(research.getBaseSecondsToResearch());
        long secondsForLocalTimeConversion = ChronoUnit.SECONDS.between(LocalDateTime.now(), this.currentResearchFinishTime);
        this.researchTimeLeft = (LocalTime.of(0, 0, 0).plusSeconds(secondsForLocalTimeConversion));
        super.resourceManager.subtractResources(super.getVillage(), research.getResourcesRequiredToResearch());
    }
    ...
    private void checkIsAbaleToStartResearch(Research research) {
        ...
        if (!super.resourceManager.hasEnoughResourcesAvailable(super.getVillage(), research.getResourcesRequiredToResearch())) {
    ...
    protected void updateVillageState() {
        ...
        if (LocalDateTime.now().isAfter(this.currentResearchFinishTime)) {
            ...
            super.getVillage().addToVillageLog(LocalDateTime.now(),
                    "Finished " + this.currentResearch.getResearchName() + " research to level " + this.currentResearch.getResearchLevel());
