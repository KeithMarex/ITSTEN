...
public abstract class Building {
    ...
    @Getter
    @Setter
    private boolean producesResources = false;
    @Getter
    @Setter
    @Transient
    private Map<String, Integer> buildingLevelRequiredToLevelup = new HashMap<>();
    @Setter
    private LocalTime constructionTimeLeft;
    @Getter
    @Setter
    private LocalTime constructionTime;
    @Getter
    @Setter
    private LocalDateTime levelupFinishedTime;
    @Getter
    private long constructionTimeSeconds = 0;
    @Getter
    @Setter
    ...
    @Setter
    @Getter
    private int populationRequiredNextLevel;

    @Setter
    @Getter
    private int totalPopulationInUse;
    ...
    public LocalTime getConstructionTimeLeft() {
        ...
        long secondsTimeLeft = LocalTime.now().until(this.levelupFinishedTime.toLocalTime(), ChronoUnit.SECONDS);
    ...
    public void levelUp() {
        if (!this.resourceManager.hasEnoughResourcesAvailable(this.village, this.resourcesRequiredLevelUp)) {
    ...
    public void updateBuildingState() {
        ...
        this.populationRequiredNextLevel = this.level == 0 ? this.getPopulationRequired(1) :
                this.getPopulationRequired(this.level + 1);
        //When the building is under construction the population of the next level needs to be added already.
        this.totalPopulationInUse = this.getPopulationRequired(this.level + (this.isUnderConstruction ? 1 : 0));
        this.updateBuilding();
    }
    ...
    private void checkLevelUpFinished() {
        if (this.levelupFinishedTime != null && LocalDateTime.now().isAfter(this.levelupFinishedTime)) {
            ...
            this.village.addToVillageLog(LocalDateTime.now(), "Finished building " + this.getClass().getSimpleName());
